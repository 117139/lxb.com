...
class Base extends Controller
{
     ...
     * 检查交易状态
     */
    public function check_deal($type=0)
    {
        $uid = session('user_id');
        $uinfo = db('xy_users')->field('deal_status,status,balance,level,deal_count,deal_time,deal_reward_count dc,fudan_deal_count')->find($uid);
        ...
        if($uinfo['deal_time']==strtotime(date('Y-m-d'))){
            ...
            if($type){
                if ($uinfo['fudan_deal_count'] >= $ulevel['fudan_order_num']) {
