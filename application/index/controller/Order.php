    ...
    public function order_info()
    {
        if(\request()->isPost()){
            ...
            $oinfo = db('xy_convey')
                        ...
                        ->field('xc.id oid,xc.addtime,xc.commission,xc.num,xc.goods_count,xc.add_id,xg.goods_name,xg.cid,xg.goods_price,xg.shop_name,xg.goods_pic,ar.name,ar.tel,ar.address,u.balance')
                        ->where('xc.id',$oid)
                        ->where('xc.uid',session('user_id'))
                        ->find();
            if(!$oinfo) return json(['code'=>1,'暂无数据']);
            $oinfo['endtime'] = date('Y/m/d H:i:s', $oinfo['addtime'] + config('deal_timeout') );
