...
//        $invite_code = $tel;
    ...
    //自动升级
    public function auto_vip($uid)
    {
        $user = self::parent_user($uid,5);
        if ($user) {
            foreach ($user as $item) {
                $uid = $item['id'];
                $uinfo = db('xy_users')->find($uid);
                $level = db('xy_level')->order('level desc')->select();
                $userChilds = self::child_user($uid,5);
                foreach ($level as $v) {
                    $level = $v['level'];
                    $auto_vip_xu_num = $v['auto_vip_xu_num'];
                    if ( count($userChilds) >= $auto_vip_xu_num) {
                        if ($level>$uinfo['level']) {
                            db('xy_users')->where('id',$uid)->update(['level'=>$level]);
                            break;
                        }
                    }
                }
     ...
     * @return array
     */
    public function edit_users($id,$tel,$user_name,$pwd,$pwd2,$level,$parent_id,$balance,$freeze_balance,$token){
...
//        }
        ...
        $qrCode->saveImage($dir . '/' . $user_id . '.png');
        ...
        $image->watermark($qr, [255,173])->text($invite_code,\Env::get('root_path').'public/public/fz.TTF',22,'#000000',[(678-(24*strlen($user_id)))/2,945])->save(\Env::get('root_path').'public/upload/qrcode/user/'.$n.'/'.$user_id.'-1.png');
    }

    public function infoById($id)
    {
        $uinfo = Db::name('xy_users')->field(['id', 'balance'])->find();
        $uinfo['ip'] = $this->get_server_ip();
        return $uinfo;
    }
